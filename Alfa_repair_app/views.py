...
def export_serials_to_excel(request):
    ...
    # Заголовки
    headers = [
        'ID', 'Batch ID', 'Серийный номер', 'Модель банка', 'Модель',
        'Производитель', 'Статус', 'Номер коробки', 'Трек в ремонт',
        'Трек из ремонта', 'Трек в банк', 'Местонахождение'
    ]
    ws.append(headers)

    # Получаем все записи
    serials = SerialNumber.objects.all().select_related('batch')

    # Заполняем строки
    for s in serials:
        ws.append([
            s.id,
            s.batch.id if s.batch else '',
            s.serial,
            s.model_bank,
            s.model or '',
            s.brand or '',
